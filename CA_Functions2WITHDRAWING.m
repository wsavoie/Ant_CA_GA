...
if(DRAW)
                ...
                set(gcf,'position',[100 500 1000 80]);
v = VideoWriter('active','Uncompressed AVI');
v.FrameRate=50;
  open(v)
...
for z=1:size(numantsALL,2)
    ...
    for run=runIts
        clearvars -except numants DRAW run workfolder schRes ResEnergy groupEnergy...
            indEnergy tunLength numantsALL z proba tt tw runIts energyMult cmap rech ptt iterations tuntip v
        countSize= 4;
...
%         end
        ...
        systemState.activeTrip=zeros(2,numants);
...
%                 pts(systemState.prob);
        ...
        for kk=1:iterations
            ...
            systemState.activeTrip(1,:)=systemState.activeTrip(1,ind);
            systemState.activeTrip(2,:)=systemState.activeTrip(2,ind);
            ...
            for jj=1:numants
                ...
                %%%%%%%%%%ACTIVE TRIP%%%%%%%%%%%%%%%%%%%%
                %if resting again, reset active trip flag
                if(systemState.restflag(jj)==1)
                    systemState.activeTrip(2,jj)=0;
                end
                
                %if within tip spot add one to activity if hasn't been
                %added yet
                if(x(jj)<roadlength-tunneltip-1+tipSpot)
                    if(systemState.activeTrip(2,jj)==0)
                        systemState.activeTrip(1,jj)=systemState.activeTrip(1,jj)+1;
                        systemState.activeTrip(2,jj)=1;
                    end
                end
                %%%%%%%%%%ACTIVE TRIP%%%%%%%%%%%%%%%%%%%%%
...
%                         markmatrN0(kk,systemState.globalindx(jj)+1)=1;
            ...
            if(DRAW && mod(kk,1)==0)
                ...
                %                 drawnow;
                       ...
                       %tunnel=[1 1 1];
                       %soil=[ 0.9290    0.6940    0.1250];
                       %desending ant=[0.8500    0.3250    0.0980]
                       %ascending ant=[0.6350    0.0780    0.1840]
                       ccc=jet(numants);
                    cmap=[ 0.9290    0.6940    0.1250; %soil  
                            1         1         1;%tunnel
                            0.5    0.5    0.5;%ascending
                            0.8500    0.3250    0.0980;%descending
                            0         0         0;%nest
                            ccc];
                colormap(cmap);
                               axis([0.5 roadlength+0.5 0.5 2.5]);
                newR = road+1;
                for(i = 1:numantsALL)
...
%                     else
                        ...
                        vdir2=road(systemState.y(id), systemState.x(id))/abs(road(systemState.y(id), systemState.x(id)))/2;
                        newR(systemState.y(id),systemState.x(id))=3.5+vdir2;
%                     end
%                         newR(systemState.y(id),systemState.x(id))=5+i;
                        newR(1,roadlength)=5;
                        newR(2,roadlength)=5;
                end
...
%                 for(i = 1:numantsALL)
%                      id = find(systemState.globalindx==i);
%                      if(systemState.pellet(id)==1&& systemState.x(id)~=roadlength)
%                         plot(systemState.x(id),systemState.y(id),'s','color',[0.9290    0.6940    0.1250]...
%                             ,'markersize',7,'markerfacecolor',[0.9290    0.6940    0.1250]);
%                      end
%                 end
...
%                 text(10,1,['t = ',num2str(kk/2/60,'%10.2g'),' min'],'fontsize',20);
                ...
                %
                for(i=0:roadlength)
                      plot([i+0.5,i+0.5],[0.5,2.5],'k','linewidth',1.2);
                end
                    plot([roadlength+1,0],[1.5,1.5],'k','linewidth',1.2);
                    plot([roadlength+1,0],[0.5,0.5],'k','linewidth',1.2);
                    plot([roadlength+1,0],[2.5,2.5],'k','linewidth',1.2);
                drawnow;
                
                fff=getframe(gcf);
                writeVideo(v,fff);
%                 saveas(gcf,['A:\PAPER FIGS and data\lorenzProtocol\', num2str(kk)],'png')
    ...
    ResEnergy(z).equalDis     = all(probs(1,1)==probs(1,:));
    ResEnergy(z).ActiveTrip= systemState.activeTrip(1,:);
