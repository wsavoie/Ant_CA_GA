...
temp_frame=test; %use this if ant identity preserved
test(find(test==1))=0;
test(find(test>0))=1;
test(find(test<0))=1;
...
for i=1:size(clusters,2)
    if (clusters(i)>1)
        ...
        if (clusters(i)==2)
            ...
            %use this if ant identity preserved
            for m=1:1:numel(clusters_structure(i,j).coordinatex)
            clusters_structure(i,j).ants(m)=temp_frame(clusters_structure(i,j).coordinatex(m),clusters_structure(i,j).coordinatey(m));
            end
    ...
    %use this if ant identity preserved
    for m=1:1:numel(clusters_structure(1,j).coordinatex)
    clusters_structure(1,j).ants(m)=temp_frame(clusters_structure(1,j).coordinatex(m),clusters_structure(1,j).coordinatey(m));
    end
